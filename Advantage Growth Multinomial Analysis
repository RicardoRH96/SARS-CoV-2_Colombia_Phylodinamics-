...
GISAID = read.csv("GISAID.csv", sep = ",")
...
GISAID$pango_lineage[grepl("B.1.1.7", GISAID$pango_lineage)] = "Alpha"
GISAID$pango_lineage[grepl("B.1.621|B.621.1|B.1.621.2|BB.2",GISAID$pango_lineage)] = "Mu"
GISAID$pango_lineage[grepl("P.1",GISAID$pango_lineage)] = "Gamma (P.1)"
GISAID$pango_lineage[grepl("B.1.617.2|AY",GISAID$pango_lineage)] = "Delta (B.1.617.2 & AY.X)"
GISAID$pango_lineage[grepl("B.1.1.529|BA.1", GISAID$pango_lineage)] = "Omicron"
GISAID$pango_lineage[grepl("C.37", GISAID$pango_lineage)] = "Lambda"
...
tblB1621 = table_country_lineage[grepl(sel_target_VOC, table_country_lineage$Lineage, fixed=T)&table_country_lineage$Count>10,]
tblB1621

sel_ref_lineage = "Gamma (P.1)"
...
tblP1 = table_country_lineage[table_country_lineage$Lineage==sel_ref_lineage&table_country_lineage$Count>10&table_country_lineage$Country %in% sel_countries,]
tblP1
...
nrow(GISAID_sel) # 11270
range(GISAID_sel$date) # "2021-01-01" "2021-08-20"
...
nrow(GISAID_sel) # 12888


sum(GISAID_sel$LINEAGE2==sel_target_VOC) # 4650
sum(GISAID_sel$LINEAGE2=="Alpha") # 198
sum(GISAID_sel$LINEAGE2=="Lambda") # 1
...
VOCs = c("Delta (B.1.617.2 & AY.X)", sel_target_VOC,"Lambda","Alpha", "Gamma (P.1)","Omicron")
...
remove2 = remove2[!(remove2 %in% c("Mu","Alpha","Gamma (P.1)","Delta (B.1.617.2 & AY.X)", "Omicron", "Lambda"))]
GISAID_sel$LINEAGE2[(GISAID_sel$LINEAGE2 %in% remove2)] = "other" # minority VOCs
GISAID_sel$LINEAGE2 = factor(GISAID_sel$LINEAGE2)
GISAID_sel$LINEAGE2 = relevel(GISAID_sel$LINEAGE2, ref="Gamma (P.1)") # we code UK strain as the reference level
levels(GISAID_sel$LINEAGE2)
# [1] "B.1.1.7"                  "B.1.1.519"                "B.1.351"                  "B.1.617.1"                "C.1.2"                   
# [6] "Delta (B.1.617.2 & AY.X)" "other" 
levels_LINEAGE2 = c("Alpha","B.1.1.348",
                    "B.1.625","Delta (B.1.617.2 & AY.X)",sel_target_VOC,"Gamma (P.1)","B.1", "B.1.111", "other", "Omicron", "Lambda")
...
sum(data_agbyday2[data_agbyday2$LINEAGE2=="B.1.621.2","total"]) == nrow(GISAID_sel) # correct
...
sum(data_agbyweek2[data_agbyweek2$LINEAGE2=="B.1.621.1","total"]) == nrow(GISAID_sel) # correct
...
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Alpha")] = "#0085FF"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Lambda")] = "#9A9D00"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Gamma (P.1)")] = "cyan3"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="Omicron")] = muted("magenta")
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)==sel_target_VOC)] = "magenta"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1")] = "#00CED1"
lineage_cols2[which(levels(GISAID_sel$LINEAGE2)=="B.1.111")] = "#800000"
...
muller_colombia_raw = ggplot(data=data_agbyweek2, aes(x=collection_date, y=count, group=LINEAGE2)) +
  ...
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01",  "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01"))),1,1),
...
ggsave(filename = "Muller_prev.jpg", plot = muller_colombia_raw, height = 20, width = 25, units = "cm", dpi = 900)
...
#                    df      BIC
#fit1_colombia_multi 20 23147.87
#fit2_colombia_multi 30 21011.89
#fit3_colombia_multi 40 20739.28
#fit4_colombia_multi 50 20574.30


# growth rate advantage compared to Delta in overall model (difference in growth rate per day) 
emtrcolombia = emtrends(fit4_colombia_multi, trt.vs.ctrl ~ LINEAGE2,  
...
multinom_preds_today_avg = data.frame(emmeans(fit4_colombia_multi, ~ LINEAGE2|1,
                                              at=list(DATE_NUM=today_num), 
                                              mode="prob", df=NA))
multinom_pred_table <- multinom_preds_today_avg
#                   LINEAGE2         prob           SE df     asymp.LCL    asymp.UCL
# 1 Delta (B.1.617.2 & AY.X) 9.832471e-01 7.986567e-03 NA  9.675937e-01 9.989005e-01
# 2                  B.1.1.7 5.605154e-05 3.097777e-05 NA -4.663769e-06 1.167669e-04
# 3                B.1.1.519 3.020925e-32 2.291057e-30 NA -4.460181e-30 4.520599e-30
# 4                  B.1.351 6.422437e-05 2.560654e-05 NA  1.403646e-05 1.144123e-04
# 5                B.1.617.1 1.316065e-06 3.698963e-06 NA -5.933769e-06 8.565900e-06
# 6                    C.1.2 1.563474e-02 7.986229e-03 NA -1.798573e-05 3.128746e-02
# 7                    other 9.965396e-04 3.606689e-04 NA  2.896415e-04 1.703438e-03

# % non-Delta
colSums(multinom_preds_today_avg[-1, c("prob","asymp.LCL","asymp.UCL")])
#               prob asymp.LCL asymp.UCL 
# 0.0167528675 0.0002750947 0.0332306403
...
fit_colombia_multi_preds = data.frame(predgrid, as.data.frame(predict(fit4_colombia_multi, newdata=predgrid, type="prob")),check.names=F)
library(tidyr)
library(tidyselect)
...
ggsave("muller_plot_pred.jpg", plot = muller_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)
...
fit_colombia_multi_preds_withCI_overall = data.frame(emmeans(fit4_colombia_multi,
                                                                ~ LINEAGE2,
                                                                by=c("DATE_NUM"),
...
muller_colombia_mfit2 = ggplot(data=fit_colombia_multi_preds_withCI_overall, 
  ...
  annotate("rect", xmin=max(GISAID_sel$DATE_NUM)+1, 
           xmax=as.Date(date.to, origin="1970-01-01"), ymin=0, ymax=1, alpha=0.4, fill="white") + # extrapolated part
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01"))),1,1),
                     limits=as.Date(c("2020-03-01",NA)), expand=c(0,0)) +
  # guides(color = guide_legend(reverse=F, nrow=1, byrow=T), fill = guide_legend(reverse=F, nrow=1, byrow=T)) +
  theme_minimal() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
...
ggsave("muller_pred_smoothed.jpg", plot = muller_colombia_mfit2, width = 25, height = 20, units = "cm", dpi = 900)


library(ggpubr)
ggarrange(muller_colombia_raw2 + coord_cartesian(xlim=c(as.Date("2020-03-01"),as.Date(date.to, origin="1970-01-01")))+
            theme(legend.background = element_rect(fill = alpha("white", 0)),
                  legend.key = element_rect(fill = alpha("white", 0)),
                  legend.text=element_text(color = "white")) +
            guides(colour = guide_legend(override.aes = list(alpha = 0)),
...
muller_colombia_mfit_overall = ggplot(data=fit_colombia_multi_preds2, 
  ...
  annotate("rect", xmin=max(GISAID_sel$DATE_NUM)+1, 
           xmax=as.Date(date.to, origin="1970-01-01"), ymin=0, ymax=1, alpha=0.4, fill="white") + # extrapolated part
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01")),
                     labels=substring(months(as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01", "2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01"))),1,1),
                     limits=as.Date(c("2020-03-01",NA)), expand=c(0,0)) +
  # guides(color = guide_legend(reverse=F, nrow=1, byrow=T), fill = guide_legend(reverse=F, nrow=1, byrow=T)) +
  theme_minimal() + theme(legend.position="right", 
                     axis.title.x=element_blank()) + 
...
ggsave("mnomial_logit.jpg", plot = plot_colombia_mfit_logit, height = 20, width = 25, units = "cm", dpi = 900)
...
ggsave("mnomial_response.jpg", plot = plot_colombia_mfit, height = 20, width = 25, units = "cm", dpi = 900)
...
ggsave("fitted_cases_raw.jpg", plot = fitted_cases_raw, height = 20, width = 25, units = "cm", dpi = 900)
...
ggsave("fitted_cases_smoothed.jpg", plot = fitted_cases_smoothed, height = 20, width = 25, units = "cm", dpi = 900)
...
qplot(data=avg_r_cases, x=DATE-7, y=Re, ymin=Re_LOWER, ymax=Re_UPPER, geom="ribbon", alpha=I(0.5), fill=I("steelblue")) +
  ...
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01","2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01")),
                     labels=c("M","A","M","J","J","A","S","O","N","D","J","F","M","A","M","J","J","A","S", "O", "N", "D", "J", "F")) +
...
r_variants <- qplot(data=above_avg_r_variants2[!((above_avg_r_variants2$variant %in% c("other"))|above_avg_r_variants2$collection_date>max(cases_tot$DATE)),], 
  ...
  scale_x_continuous(breaks=as.Date(c("2020-01-01","2020-02-01","2020-03-01","2020-04-01","2020-05-01","2020-06-01","2020-07-01","2020-08-01","2020-09-01","2020-10-01","2020-11-01","2020-12-01","2021-01-01","2021-02-01","2021-03-01","2021-04-01","2021-05-01","2021-06-01","2021-07-01","2021-08-01","2021-09-01","2021-10-01", "2021-11-01", "2021-12-01", "2022-01-01", "2022-02-01")),
                     labels=c("J","F","M","A","M","J","J","A","S","O","N","D","J","F","M","A","M","J","J","A","S", "O", "N", "D", "J", "F")) +
  # scale_y_continuous(limits=c(1/ymax,ymax), trans="log2") +
  geom_hline(yintercept=1, colour=I("red")) +
  ggtitle("Re VALUES OF SARS-CoV2 VARIANTS IN COLOMBIA (based on case data & multinomial fit to\nGISAID data)") +
  # labs(tag = tag) +
  # theme(plot.margin = margin(t = 20, r = 10, b = 20, l = 0)) +
  theme(plot.tag.position = "bottomright",
        plot.tag = element_text(vjust = 1, hjust = 1, size=8)) +
...
ggsave("R_variants.jpg", plot = r_variants, width = 25, height = 20, units = "cm", dpi = 900)
...
emtrcol_pairw = emtrends(fit4_colombia_multi, pairwise ~ LINEAGE2,  
                             var="DATE_NUM",  mode="latent",
...
gtsave(r_gt, filename = "r_table.png", expand = 10, path = "~/Documents/R scripts/Phylodynamics")
gtsave(multinom_gt, filename = "mnom_table.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics")
gtsave(pairwise_r, filename = "pwise_r_table.pdf", expand = 10, path = "~/Documents/R scripts/Phylodynamics")
